...
namespace Habanero.WebGUI
{
    ...
    public class GridLayoutManager : LayoutManager
    {
        ...
        public IList Rows
        {
            get
            {
                ...
                for (int i = 0; i < RowCount; i++)
                {
                    ...
                    {
                        if ((i*ColumnCount + j) < _controls.Count)
                        {
                            row.Add(_controls[i*ColumnCount + j]);
                        }
                        else
                        {
                            row.Add(null);
        ...
        public IList Columns
        {
            get
            {
                ...
                for (int i = 0; i < ColumnCount; i++)
                {
                    ...
                    {
                        if ((ColumnCount*j + i) < _controls.Count)
                        {
                            col.Add(_controls[ColumnCount*j + i]);
                        }
                        else
                        {
                            col.Add(null);
        ...
        public Control AddControl(Control control, int rowSpan, int columnSpan)
        {
            ...
            int currentColNum = (_controls.Count)%ColumnCount;
            int currentRowNum = (_controls.Count)/ColumnCount;
            ...
            ManagedControl.Controls.Add(control);
            _controlInfoTable.Add(control, new ControlInfo(control, columnSpan, rowSpan));
        ...
        protected override void RefreshControlPositions()
        {
            ...
            for (int i = 0; i < _controls.Count; i++)
            {
                ...
                if ((i > 0) && (currentCol == 0))
                {
                    ...
                    _currentPos.Y += _controls[i - 1].Height + GapSize;
                }
                ctl.Left = _currentPos.X;
                ctl.Top = _currentPos.Y;
                int width = 0;
                ControlInfo ctlInfo = (ControlInfo) _controlInfoTable[ctl];
                for (int cols = currentCol; cols < Math.Min(ColumnCount, currentCol + ctlInfo.ColumnSpan); cols++)
                {
                ...
                width += (GapSize*(ctlInfo.ColumnSpan - 1));
                ctl.Width = width;
        ...
        private static int GetFixedAmount(int[] arr)
        {
            int total = 0;
        ...
        /// <returns>Returns the count</returns>
        private static int GetNumVariableEntries(int[] arr)
        {
            int num = 0;
            for (int i = 0; i < arr.Length; i++)
            {
