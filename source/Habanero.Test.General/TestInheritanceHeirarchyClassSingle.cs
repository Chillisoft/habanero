...
//{
    /// <summary>
    /// NB: These tests are failing now.  Will fix the code to get this working when
    /// it is actually needed by a client (basically, the code is climbing up the
    /// inheritance heirarchy when it detects ClassTableInheritance, but it doesn't
    /// go walking upwards when there is singleTI, so you're only getting the set
    /// of statements for the bottom-most class in this heirarchy)
    /// PS. the tests need to be fixed too - the setup should be OK in terms of
    /// the classes, but the actual tests need to be fixed and the DB needs a new
    /// ShapeType column in Circle.
    /// 
    /// Tests heirarchical inheritance where FilledCircle inherits from Circle using
    /// SingleTableInheritance and Circle inherits from Shape using ClassTableInheritance.
    /// In other words, two database tables exist: one that stores Shape
    /// and the other other that stores Circle and FilledCircle.  The implication is that FilledCircle
    /// has no FilledCircleID, meaning FilledCircle has a foreign key to CircleID.
    /// </summary>
    //[TestFixture]
    //public class TestInheritanceHeirarchyClassSingle : TestInheritanceHeirarchyBase
    //{
        //[TestFixtureSetUp]
        //public void SetupFixture()
        //{
        //    SetupTestForFilledCircleNoPrimaryKeyInheritsCircle();
        //}
        ...
        //    FilledCircleNoPrimaryKeyInheritsCircle.GetClassDef().SuperClassDef =
        //        new SuperClassDef(Circle.GetClassDef(), ORMapping.SingleTableInheritance);
        //    FilledCircleNoPrimaryKeyInheritsCircle.GetClassDef().SuperClassDef.Discriminator = "ShapeType";
        //}

        //protected override void SetStrID()
        //{
        //    itsFilledCircleId = (string) DatabaseUtil.PrepareValue(itsFilledCircle.GetPropertyValue("CircleID"));
        //}
        ...
        //    Assert.AreEqual(ORMapping.SingleTableInheritance, FilledCircleNoPrimaryKeyInheritsCircle.GetClassDef().SuperClassDef.ORMapping);
        //}

        //[Test]
        //public void TestFilledCircleHasCircleIDAsPrimaryKey()
        //{
        //    try
        //    {
        //        Assert.IsTrue(itsFilledCircle.ID.Contains("CircleID"));
        ...
        //    {
        //        Assert.Fail("An object using SingleTableInheritance should have the super class's primary key.");
        //    }
        //}
        ...
        //    Assert.AreEqual("DELETE FROM Circle WHERE CircleID = ?Param0",
