...
namespace Habanero.Test.Bo
{
    ...
    public class TestPropRuleDecimal
    {
        ...
        public void TestDecimalRule()
        {
            ...
            //Test less than min
            Assert.IsFalse(rule.isPropValueValid(5.31116m, ref errorMessage));
            Assert.IsTrue(errorMessage.Length > 0);
            //Test valid data
            Assert.IsTrue(rule.isPropValueValid(6, ref errorMessage));
            Assert.IsFalse(errorMessage.Length > 0);
            //test greater than max
            Assert.IsFalse(rule.isPropValueValid(10.1111112m, ref errorMessage));
            Assert.IsTrue(errorMessage.Length > 0);
            ...
            Assert.IsTrue(errorMessage.Length == 0);
            errorMessage = "";
            Assert.IsFalse(rule.isPropValueValid(-53444.33222m, ref errorMessage));
            Assert.IsTrue(errorMessage.Length > 0);
        }
    }
}
